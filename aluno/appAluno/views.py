...
from aluno.appAno.models import Ano
from aluno.appClasse.models import Classe
from aluno.appMatricula.models import Matricula
from utilitarios.utilitarios import (criarMensagem, padronizar_nome,
                                     retornarNomeMes)


from aluno.appAluno.forms import frmAluno

from aluno.appAluno.models import Aluno
from aluno.appAluno.models import Telefone
...
def atualizarTabela(alunos):
    ...
    for aluno in alunos:
        ...
        #botao_matricula_ia = f'<button type="button" class="{classe_ia} ia" title="Matrícula por Idade" value="{aluno.rm}">\
        #         {icon_ia}\
        #         </button>'
        #                        <td class="align-middle text-center">{botao_matricula_ia}</td>
...
def index(request):
    ...
    return render(request, 'appAluno/index.html', context)
...
def baixar_declaracao(request):
    ...
    # -----------------------------------------------------
    header_img = Image(
        "aluno/appAluno/static/appAluno/jpeg/cabecalho_600dpi.png",
        width=500,
        height=120,
    )
